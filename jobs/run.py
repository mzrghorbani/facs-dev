...
if __name__ == '__main__':
    ...
    simulation_period = int(sys.argv[6])
    ...
    outfile = "{}-{}-{}.csv".format(region, transition_scenario, transition_day)
    ...
    e.ages = read_age_csv.read_age_csv("age-distr.csv", facs.region)

    print("age distribution in system:", e.ages, file=sys.stderr)

    e.disease = read_disease_yml.read_disease_yml("disease_covid19.yml")

    building_file = "{}_buildings.csv".format(facs.region)
    read_building_csv.read_building_csv(e,
                                        building_file,
                                        "building_types_map.yml",
                                        house_ratio=house_ratio, workspace=12, office_size=1600, household_size=2.6,
    ...
    measures_file = "measures_{}.yml".format(facs.region)
