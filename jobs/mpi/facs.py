...
lids = {"park": 0, "hospital": 1, "supermarket": 2, "office": 3, "school": 4, "leisure": 5,
        "shopping": 6}  # location ids and labels
        ...
        with open(csvfile) as csvfile:
            ...
            for row in needs_reader:
                if row_number == 0:
                    ...
                    self.needs[i, lids["school"]] = int(0.75 * int(
                        row[needs_cols[i]]))  # assuming 25% of school time is outside of the building (PE or breaks)
                row_number += 1
...
def log_infection(t, x, y, loc_type, r):
    global num_infections_today
    out_inf = out_files.open("{}_covid_out_infections.csv".format(r))
    print("{},{},{},{}".format(t, x, y, loc_type), file=out_inf, flush=True)
    num_infections_today += 1


def log_hospitalisation(t, x, y, age, r):
    global num_hospitalisations_today
    out_inf = out_files.open("{}_covid_out_hospitalisations.csv".format(r))
    print("{},{},{},{}".format(t, x, y, age), file=out_inf, flush=True)
    num_hospitalisations_today += 1


def log_death(t, x, y, age, r):
    global num_deaths_today
    out_inf = out_files.open("{}_covid_out_deaths.csv".format(r))
    print("{},{},{},{}".format(t, x, y, age), file=out_inf, flush=True)
    num_deaths_today += 1


def log_recovery(t, x, y, age, r):
    global num_recoveries_today
    out_inf = out_files.open("{}_covid_out_recoveries.csv".format(r))
    print("{},{},{},{}".format(t, x, y, age), file=out_inf, flush=True)
...
class Person:
    def __init__(self, location, household, ages):
        ...
        self.job = np.random.choice(4, 1, p=[0.865, 0.015, 0.08, 0.04])[
            0]  # 0=default, 1=teacher (1.5%), 2=shop worker (8%), 3=health worker (4%)
    ...
    def vaccinate(self, time, vac_no_symptoms, vac_no_transmission, vac_duration):
        ...
        if vac_duration > 0:
            if vac_duration > 100:
                self.phase_duration = np.random.gamma(10,
                                                      vac_duration / 10.0)  # shape parameter is changed with variable, shape parameter is kept fixed at 15 (assumption).
            else:
    ...
    def infect(self, t, severity="exposed", location_type="house"):
        ...
        self.mild_version = True
        self.hospitalised = False
        log_infection(t, self.location.x, self.location.y, location_type, region)

    def recover(self, e, t, location):
        if e.immunity_duration > 0:
            self.phase_duration = np.random.gamma(e.immunity_duration / 15,
                                                  15)  # shape parameter is changed with variable, shape parameter is kept fixed at 15 (assumption).
        self.status = "recovered"
        self.status_change_time = t
        log_recovery(t, self.location.x, self.location.y, location, region)
            ...
            # non-mild version (will involve ICU visit)
                ...
                if not self.hospitalised:
                    if t - self.status_change_time >= self.phase_duration:
                        ...
                        log_hospitalisation(t, self.location.x, self.location.y, self.age, region)
                        self.status_change_time = t  # hospitalisation is a status change, because recovery_period is from date of hospitalisation.
                        if get_rnd() < self.get_mortality_chance(disease) / self.get_hospitalisation_chance(
                    ...
                    if t - self.status_change_time >= self.phase_duration:  # from hosp. date
                        ...
                        if self.dying:
                            self.status = "dead"
                            log_death(t, self.location.x, self.location.y, "hospital", region)
                        # hospital discharge
                        else:
            ...
            if l not in e.locations.keys():
                ...
                for k, element in enumerate(e.locations[l]):  # using 'element' to avoid clash with Ecosystem e.
                    ...
                    if element.sqm > 0:
                        ...
                        print("name: {}, x: {}, y: {}, position in array: {}, type: {}".format(element.name, element.x,
                                                                                               element.y, k, l))
...
class Location:
    ...
    def evolve(self, e, deterministic=False):
        ...
        base_rate = e.contact_rate_multiplier[self.type] * (e.disease.infection_rate / 360.0) * (
                1.0 / minutes_opened) * (e.loc_inf_minutes[self.loc_inf_minutes_id] / self.sqm)
...
class Ecosystem:
    def __init__(self, region, duration, needsfile="needs.csv", mode="parallel"):
        self.locations = {}
        self.houses = []
        self.house_names = []
        self.time = 0
        ...
        out_inf = open("{}_covid_out_infections.csv".format(region), 'w')
            ...
            for row in near_reader:
                ...
                for j in range(0, len(header_row)):
                    try:
                        ...
                        print("self.locations [keys][]", self.locations.keys(), " [][len]",
                    ...
                    for a in hh.agents:
                        ...
                        if self.vaccinations_available - self.vaccinations_today > 0:
                            if a.age > self.vaccinations_legal_age_limit and check_vac_eligibility(a) == True:
                                a.vaccinate(self.time, self.vac_no_symptoms, self.vac_no_transmission,
                                            self.vac_duration)
                                self.vaccinations_today += 1
    ...
    def print_status(self, outfile, silent=False):
        ...
        if not silent:
            ...
            print("{},{},{},{},{},{},{},{},{},{},{}".format(self.time, status["susceptible"], status["exposed"],
                                                            ...
                                                            self.num_hospitalised), file=out, flush=True)
        self.status = status
